...
void CH58X_BLEInit(void)
{
    ...
    cfg.tsCB = HAL_GetInterTempValue; // 根據溫度變化校準RF和內部RC( 大於7攝氏度 )
  #if(CLK_OSC32K)
    cfg.rcCB = Lib_Calibration_LSI; // 內部32K時鐘校準
  #endif
#endif
...
#else
    {
        ...
        for(i = 0; i < 6; i++)
        {
            cfg.MacAddr[i] = MacAddr[i]; // 使用晶片mac地址
        }
    }
#endif
...
tmosEvents HAL_ProcessEvent(tmosTaskID task_id, tmosEvents events)
{
    ...
    { // 處理HAL層訊息tmos_msg_receive讀取訊息，處理完成後刪除訊息
    ...
    {
#if(defined BLE_CALIBRATION_ENABLE) && (BLE_CALIBRATION_ENABLE == TRUE) // 校準任務，單次校準耗時小於10ms
        BLE_RegInit();                                                  // 校準RF
  #if(CLK_OSC32K)
        Lib_Calibration_LSI(); // 校準內部RC
  #endif
...
void HAL_Init()
{
    ...
    tmos_start_task(halTaskID, HAL_REG_INIT_EVENT, MS1_TO_SYSTEM_TIME(BLE_CALIBRATION_PERIOD)); // 添加校準任務，單次校準耗時小於10ms
#endif
    //  tmos_start_task( halTaskID, HAL_TEST_EVENT, 1600 );    // 添加一個測試任務
}

/*******************************************************************************
 * @fn      HAL_GetInterTempValue
 *
 * @brief   獲取內部溫感採樣值，若使用了ADC中斷採樣，需在此函數暫時屏蔽中斷
 *
 * @return  內部溫感採樣值.
 */
//extern "C"
